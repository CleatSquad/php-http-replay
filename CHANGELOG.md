...
- Default Cassette schema version is now `version: 2`. Existing `version: 1` cassettes are transparently loaded by `JsonCassetteStore` via backward-compatibility reader.
- `HttpReplayEngine` now delegates exchange selection to `ExchangeSelectorInterface`.
...
- `SequentialExchangeSelector` and `UnorderedExchangeSelector` implementations.
- Schema `version: 2` support in `JsonCassetteStore` with automated SHA-256 checksum verification and `flock` write-lock for race-condition prevention in `RecordOnce` mode.
